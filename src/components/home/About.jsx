...
export default class AboutMe extends React.Component {
  ...
  render() {
    ...
    let nextButton = classnames('about-next-button', {
      hidden:
        this.state.count == content.about.length - 1 || this.state.displayVideo,
    })
        ...
        {this.state.displayVideo ? (
          ...
          <div>
            <div className='row even-space'>
              <button className={prevButton} onClick={this.prevItem}>
                previous
              </button>
              <h2> Who Is Cody Douglass?</h2>
              <button className={nextButton} onClick={this.nextItem}>
                next
              </button>
            </div>
            <div className='row'>
